...
for p in (COW_GPIO_PIN, SHEEP_GPIO_PIN, HORSE_GPIO_PIN):
        ...
        if largest_obj is not None:
           ...
           # print(f"Largest kept: {largest_label} area={largest_area:.0f} (removed {removed})")
            ...
            if largest_label=="cow":
                ...
                GPIO.output(HORSE_GPIO_PIN,GPIO.HIGH)
                print("COW GPIO")
                ...
                GPIO.output(HORSE_GPIO_PIN,GPIO.LOW)
                GPIO.output(SHEEP_GPIO_PIN,GPIO.HIGH)
                GPIO.output(COW_GPIO_PIN,GPIO.HIGH)
                print("Horse GPIO")
            else:
...
def analytics_probe(pad, info, udata):
    try:
        ...
        if (cows + pigs + sheep) > 10:
            alerts_to_add.append(f"High density: {cows+pigs+sheep} animals")
